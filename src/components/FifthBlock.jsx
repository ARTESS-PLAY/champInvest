...
const FifthBlock = () => {
    ...
    const sendEmail = (e) => {
        ...
        emailjs.sendForm('service_3ar0niq', 'template_mvnuy2d', form.current, 'KEnODjQP4A_FihwyJ')
            .then(res => {setEmailText("Спасибо, ваша заявка принята. Скоро мы свяжемся с вами!"); setSuccessMail(true);})
            .catch(err => {setEmailText("Ошибка при отправке письма. Попробуйте позже"); setSuccessMail(false);})
    }
    ...
    return (
        <section className={styles['fifth-block']} id="fifthBlock">
            <div className={styles['inner-content']}>
                <h3>
                Рассчитать мою доходность<br /> и <span>оставить заявку</span>
                </h3>
                ...
                <form ref={form} onSubmit={sendEmail} className={styles['form-wrapper']}>
                    <div className={!validName ? styles['invalid-field'] : undefined}>
                        ...
                        {!validName && <span className={styles['error-span']}>Пожалуйста, укажите имя</span>}
                    </div>
                    <div className={!validEmail ? styles['invalid-field'] : undefined}>
                        <input type="text" name="email" placeholder="E-mail" />
                        {!validEmail && <span className={styles['error-span']}>Пожалуйста, укажите email</span>}
                    </div>
                    <div className={!validPhone ? styles['invalid-field'] : undefined}>
                        <ReactPhoneInput
                            ...
                            inputClass={`${styles["phone-input-container"]} ${validPhone ? styles['ok-phone'] : undefined}`}
                        ...
                        />
                        {!validPhone && <span className={styles['error-span']}>{validPhoneText}</span>}
                    </div>
                    ...
                    <button className={styles['select-button']} onClick={(e) => e.preventDefault()}>
                        <span className={`${styles['select-button-head']} ${investTimeButtonText !== "Срок инвестирования" ? styles['select-button-head-selected'] : undefined}`}>
                        ...
                        <div className={styles['select-button-options']}>
                            ...
                            >
                                <div
                                    className={`${styles['select-button-option-radio']} ${form.current?.investTime?.value == 6 ? styles['radio-active'] : undefined}`}
                                />
                            ...
                            >
                                <div
                                    className={`${styles['select-button-option-radio']} ${form.current?.investTime?.value == 12 ? styles['radio-active'] : undefined}`}
                                />
                                <span>от 12 месяцев</span>
                            </div>
                        </div>
                    </button>
                    <input type={"number"} placeholder="Сумма инвестиций (от 100 тыс.)" name="investAmount" onChange={handleInvestTotal}/>
                    <input style={{ display: "none" }} type={'submit'} value="" ref={submitButton} />
                </form>
                <div className={`${styles['total-invest']} ${total > 0 ? styles['total-visible'] : undefined}`}>
                    {total > 0 ?
                        <p>
                            Примерный итог инвестирования: <span>{total.replace(/\B(?=(\d{3})+(?!\d))/g, ".")} руб.</span>
                        </p> :
                        <span>
                ...
                <div className={styles["personal-data-agreement"]}>
                    <div
                        className={`${styles['checkbox']} ${checkBox ? styles['checkbox-active'] : undefined}`}
                ...
                <span className={`${styles['request-response']} ${!successMail ? styles['resError'] : undefined}`}>
